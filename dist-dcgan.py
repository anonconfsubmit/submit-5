## This is an adapted version of lsgan.py file in implementations/dcgan/
## The goal is to write a distributed version of the implementation of dcgan
...
def run(rank, size):
    ...
    criterion = torch.nn.BCELoss() #torch.nn.MSELoss #nn.BCELoss()
...
#                broad_t = time()
#                if rank == 0:
#                    print("Rank {} Group for this round {} ".format(rank, g))
                if rank in g:
                    broadcast_model(generator, group)
                    broadcast_model(discriminator, group)
                    done_round = False
#                    if opt.bench and rank == 0:
#                        print("Time to broadcast the model: ", time() - broad_t)
...
#            if opt.bench and rank == 0:
#                print("Time of forward pass on generator", time() - gen_t)
...
#            print("Gen image size: ", gen_imgs.size())
            g_loss.backward()
#            if opt.bench and rank == 0:
...
#            g_avg_t = time()
            ...
            if num_batches%opt.local_steps == 0 and num_batches > 0:
                if opt.weight_avg:
                    ...
                    average_models(generator, group, choose_r0, weights)		#Experiments show that doing this is bad anyway!
                else:
#                    if rank == 0:
#                        print("fl round {} Current group: ".format(fl_round), all_groups_np[fl_round%len(all_groups)])
                    average_models(generator, group, choose_r0)
                done_round = True
#            if opt.bench and rank == 0:
#                print("Time of averaging the generator: ", time() - g_avg_t)
...
#            disc_t = time()
            ...
            d_loss.backward()
#            if opt.bench and rank == 0:
#                print("Time of training of discriminator ", time() - disc_t)
...
#            d_avg_t = time()
            if num_batches%opt.local_steps == 0 and num_batches > 0:
                if opt.weight_avg:
                    ...
                    average_models(discriminator, group, choose_r0)
                done_round = True
#            if opt.bench and rank == 0:
#                print("Time of averaging the discriminator ", time() - d_avg_t)
            if rank == 0 and not choose_r0:
